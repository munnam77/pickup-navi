    ...
    <AppShell>
      <div className="max-w-5xl mx-auto animate-fadeIn">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold text-slate-900">行事管理</h1>
            <p className="text-sm text-slate-500">{db.events.length}件の行事</p>
          </div>
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
                      <span className="flex items-center gap-1"><Clock size={14} />{e.date} {e.time}</span>
                      <span className="flex items-center gap-1"><Users size={14} />{e.attendees.length}名参加</span>
                    </div>
                  ...
                  <div className="shrink-0">
                    {hasRoute ? (
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs text-emerald-600 bg-emerald-50 px-3 py-2 rounded-lg hover:bg-emerald-100 transition cursor-pointer"
                      >
                        <CheckCircle2 size={14} />ルート確認 <ChevronRight size={12} />
                      </button>
                    ) : e.status !== "completed" ? (
                      <button
                        onClick={() => router.push(`/route-planner?event=${e.id}`)}
                        className="flex items-center gap-1.5 text-xs text-orange-600 bg-orange-50 px-3 py-2 rounded-lg hover:bg-orange-100 transition cursor-pointer"
                      >
                        <Route size={14} />ルート作成
                      </button>
                    ) : (
                      <span className="flex items-center gap-1 text-xs text-slate-400">
                        <AlertCircle size={14} />ルートなし
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-1">
                    {e.attendees.slice(0, 8).map((aid) => {
                      ...
                      return (
                        <div
                          key={aid}
                          className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-500 -ml-1 first:ml-0 border-2 border-white"
                          title={member?.name}
                        >
                          {member?.name?.[0] || "?"}
                        </div>
                    ...
                    {e.attendees.length > 8 && (
                      <span className="text-xs text-slate-400 ml-1">+{e.attendees.length - 8}名</span>
                    )}
                  </div>
                </div>
              </div>
